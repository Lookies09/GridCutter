...
> **[최신 버전 다운로드 (v1.2.0)]([https://github.com/Lookies09/GridCutter/releases/tag/v1.2.0](https://github.com/Lookies09/GridCutter/releases/tag/v.1.2.0))**
...
### Backend
* **Python 3.8+**: 핵심 로직 및 이미지 처리
* **pywebview**: Python과 웹 기술 간의 브릿지 역할
* **Pillow / OpenCV**: 고성능 이미지 프로세싱
...
1. **레포지토리 클론**
    ```bash
    git clone https://github.com/Lookies09/GridCutter.git
    cd GridCutter
    ```
...
4. **앱 실행**
    ...
    pywebview는 빌드된 정적 파일을 기준으로 동작하므로, 프론트엔드를 먼저 빌드한 뒤 백엔드를 실행합니다.
...
> **주의**: 빌드 전 `frontend` 폴더에서 `npm run build`를 먼저 수행하여 `dist` 폴더를 생성해야 합니다.
...
빌드가 완료되면 `backend/dist/RiceGridApp.exe` 파일이 생성됩니다.
